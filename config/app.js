...
const ClaseConectar = require("./database");
...
app.get("/upload", (req, res) => {
  ClaseConectar.query("SELECT * FROM files", (err, rows, fileds) => {
    if (!err) {
      res.json(rows);
    } else {
      console.log(err);
...
app.get("/imagen/:id", (req, res) => {
  ...
  ClaseConectar.query(
    "SELECT imagen FROM files WHERE id = ?",
    id,
    (err, rows, fields) => {
      [{ imagen }] = rows;
...
app.post("/file", upload.single("file"), async (req, res, next) => {
  ...
  ClaseConectar.query("INSERT INTO files set ?", [filesImg]);
});
...
app.delete("/delete/:id", (req, res) => {
  const { id } = req.params;
  deleteFile(id);
  ClaseConectar.query("DELETE FROM files WHERE id=?", [id]);
...
app.post("/auth/:id", (req, res) => {
  ...
  ClaseConectar.query(
    "SELECT id, password FROM files WHERE id= ?",
    id,
    (err, rows, fields) => {
      [{ password }] = rows;
